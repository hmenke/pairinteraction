...
struct F {
    F() : path_cache(std::experimental::filesystem::temp_directory_path() / utils::unique_path()) {
        // Create cache directory
        if (std::experimental::filesystem::create_directory(path_cache)) {
            std::cout << "Cache directory "
                      << std::experimental::filesystem::absolute(path_cache).string() << " created."
                      << std::endl;
        } else {
    ...
    ~F() {
        ...
        std::experimental::filesystem::remove_all(path_cache);
    }
